...
class ABAE(nn.Module):
    ...
    def get_asp_importances(self, x):     
        ...
        z_s = F.normalize(torch.bmm(e_i, a_i).squeeze(2), dim=-1) 
        p_t = F.softmax(self.linear(z_s), dim=1) # aspect importance
        return p_t, z_s
    
    def aspects(self):
        E_n = F.normalize(self.E.weight[:-2], dim=1) 
...
class DSPN(nn.Module):
    def __init__(self, E, T, num_hiddens=100, num_layers=3, dropout=0.5):
        ...
        n_aspects, d_embed = T.shape
        self.E = nn.Embedding(n_vocab, d_embed)  
        self.T = nn.Embedding(n_aspects, d_embed)  # Aspect embedding
    ...
    def get_asp_importances(self, w2v_x):
        e_i = self.E(w2v_x).transpose(1, 2) 
        a_i = self.attention(e_i) 
        z_s = F.normalize(torch.bmm(e_i, a_i).squeeze(2), dim=-1)  
        p_t = F.softmax(self.linear(z_s), dim=1)  # aspect importance
        return p_t, z_s
    ...
    def sentiment_predict(self, w2v_x, p_t):
        ...
        for b in range(batch_size):
            '''1. asp_emb word_embedding attention'''
            # Cosine
...
class DSPN_BERT(nn.Module):
    def __init__(self, E, T):
        super(DSPN_BERT, self).__init__()
        n_vocab, d_embed = E.shape
        n_aspects, d_embed = T.shape
        self.E = nn.Embedding(n_vocab, d_embed)  
        self.T = nn.Embedding(n_aspects, d_embed)  # Aspect embedding
