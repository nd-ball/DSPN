...
class word2vec:
    ...
    def __iter__(self):
        for s in tqdm.tqdm(self.sentences, desc='training'):
            yield s.split()
    ...
    def embed(self, model_path, d_embed, max_n_vocab=None, window=5, min_count=10, workers=16):
        self.d_embed = d_embed
        if os.path.isfile(model_path):
            model = gensim.models.Word2Vec.load(model_path)
        else:
            model = gensim.models.Word2Vec(self, size=d_embed, max_final_vocab=max_n_vocab, 
        ...
        for word in sorted(model.wv.vocab):
            self.add_word(word)
            self.E.append(list(model.wv[word])) # E是每个词的向量，维度是(vocab_size+2, emb_size)，+2是<unk>和<pad>
        else: # 如果上面这个for循环没有发生break，那么就会执行else的语句
            self.add_word('<unk>')
    ...
    def aspect(self, n_aspects):
        ...
        km = MiniBatchKMeans(n_clusters=n_aspects, random_state=0)
        km.fit(self.E)
